...
class Lightcurve(object):
    ...
    def STD_Estimate(self, PSDdist=None, PSDdistArgs=None):
        ...
        self.std_est = SD_estimate(
            self.mean, self.freq[0], self.freq[-1], PSDdist, PSDdistArgs)[0]
        return self.std_est
    ...
    def Plot_Periodogram(self):
        ...
        plt.xlim([0.8 * min(self.periodogram[0]),
                  1.2 * max(self.periodogram[0])])
        plt.ylim([0.8 * min(self.periodogram[1]),
                  1.2 * max(self.periodogram[1])])
...
#                std = lightcurve.std
    ...
    for n in range(size):
        ...
        lc = Lightcurve(time, surrogate, tbin=tbin)
        lc.fft = fft
